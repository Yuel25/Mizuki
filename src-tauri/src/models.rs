...
impl From<&FeedRule> for crate::matcher::MatchRule {
    fn from(rule: &FeedRule) -> Self {
        Self {
            includes: rule.includes.clone(),
            excludes: rule.excludes.clone(),
            resolution: rule.resolution.clone(),
            subtitle_group: rule.subtitle_group.clone(),
        }
    }
}
...
pub struct RssItem {
    ...
    pub downloaded: bool,
    pub download: Option<RssDownloadStatus>,
    pub matches_rule: bool,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RssDownloadStatus {
    pub task_id: String,
    pub state: String,
    pub progress: f64,
    pub active: bool,

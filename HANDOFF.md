...
- 已完成任务无法播放（三层回退）：
  1. 任务句柄在会话内 → 直接读元数据返回文件清单（多视频弹选集）；
  2. 句柄不在（重启后）→ 读 `downloads.playback_path` 落库的主视频路径；
  3. 落库路径也没有（修复前完成的旧任务）→ 用库中的种子来源以暂停状态临时恢复到
     会话，读取文件清单后把主视频路径落库；需要联网与一次磁盘校验，句柄留在会话内。
- 详情页新增“在 Bangumi 打开 ↗”按钮（`https://bgm.tv/subject/{id}`）。

代码审查修复（2026-08-28）：

- 用户手动暂停的任务被再次添加后不再被排队逻辑吞掉（此前并发不限时会永久卡“排队中”）。
- 启动恢复改为全部暂停装入 + 统一走并发调度，重启不再绕过“同时下载数”限制；
  同时恢复改用每个任务自己保存的下载目录（此前一律用当前设置目录，改过目录会导致旧任务找不到数据）。
- 托盘“暂停全部下载”不再把已完成任务改成 paused（避免下次重启做无谓校验）。
- 删除任务时清理测速样本（轻微内存泄漏）。
